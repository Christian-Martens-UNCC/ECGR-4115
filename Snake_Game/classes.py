...
class Main:
    ...
    def update(self):
        if ALGO_PICK == 0:
            ...
            l = len(self.snake.body)
            time_shift = [1000, 1000, 1000]
            a_star_time = 0.0000207515523393911 * l + 0.0016494940970876
            a_star_acc = -0.00954738028057518 * l + 0.773800362546249
            bfs_time = -7.06275204956167E-07 * l + 0.00295929941561875
            bfs_acc = -0.0107186614720867 * l + 0.880245481601273
            dij_time = 0.0000114618131795612 * l + 0.00572529119436665
            dij_acc = -0.010937484096782 * l + 0.901377197000111

            if a_star_acc >= ACC_CONST:
                time_shift[0] = 1
            if bfs_acc >= ACC_CONST:
                time_shift[1] = 1
            if dij_acc >= ACC_CONST:
                time_shift[2] = 1

            time_shift = [a_star_time * time_shift[0],
                          bfs_time * time_shift[1],
                          dij_time * time_shift[2]]
            time_min = min(time_shift)
            time_idx = time_shift.index(time_min)
            self.algo_const = time_idx
            if time_idx == 0:
                self.a_star_algo()
            elif time_idx == 1:
                self.bfs_algo()
    ...
    def eat_fruit_check(self):
        if self.fruit.pos == self.snake.body[-1]:
            ...
            self.path_moves = 0
            if WRITING_CSV:
                self.data.append([self.restart_count, ALGO_NAMES[self.algo_const],
    ...
    def draw_info(self, screen):
        ...
        info_surface_1 = self.text_font.render(
            f"Algorithm = {ALGO_NAMES[self.algo_const]}, Length = {str(snake_len)}, Attempt = {str(self.restart_count)}, "
    ...
    def data_to_csv(self):
        self.data = self.data.append([self.restart_count, ALGO_NAMES[self.algo_const], len(self.snake.body), 1])